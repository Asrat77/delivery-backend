...
export async function calculatePrice(input: { packageType: string; weight: number }) {
  ...
  for (const rule of matching) {
    ...
    if (rule.type === "FIXED" && rule.fixedPrice !== null) return Number(rule.fixedPrice);
    if (rule.type === "PER_KG" && rule.pricePerKg !== null) return Number(rule.pricePerKg) * input.weight;
  }
