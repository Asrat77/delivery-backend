...
const service = __importStar(require("./live-tracking.service"));
...
const shipmentActiveDriver = new Map();
let io = null;
function getIO() {
    if (!io)
...
function createSocketServer(httpServer) {
    ...
    io.use(async (socket, next) => {
        try {
            ...
            socket.user = await (0, jwt_1.verifyToken)(token);
            next();
        }
        catch {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("leave:shipment", async (shipmentId) => {
...
function setupDriverSocket(io, socket, userId) {
    ...
    socket.on("driver:location", async (payload) => {
        ...
        try {
            const progress = await service.calculateProgress(activeShipmentId, payload.lat, payload.lng);
    ...
    socket.on("driver:start-delivery", async (payload) => {
        const shipment = await prisma_1.prisma.shipment.findUnique({
    ...
    socket.on("driver:status-change", async (payload) => {
        const driver = await prisma_1.prisma.driver.findUnique({ where: { userId } });
        if (!driver)
            return;
        const shipment = await prisma_1.prisma.shipment.findUnique({
