...
exports.get = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    return res.status(200).json((0, response_1.successResponse)("COD", cod));
});
exports.markCollected = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const cod = await codService.markCollected({
        shipmentId: String(req.params.shipmentId),
        userId: req.user.id,
        role: req.user.role,
    });
    return res.status(200).json((0, response_1.successResponse)("COD marked as collected", cod));
});
